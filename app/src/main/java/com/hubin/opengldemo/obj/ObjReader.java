...
public class ObjReader {
    ...
    public static List<Obj3D> readMultiObj(Context context, String file){
        ...
        HashMap<String,MtlInfo> mTls=null;  //纹理数据
...
//                    LogUtils.i("readMultiObj I : "+temps);
                    String[] tempsa=temps.split("[ ]+");//根据空格切割
                    switch (tempsa[0].trim()){
                        case "mtllib":  //材质
                            LogUtils.w("readMultiObj I 发现mtl: "+temps);
                            InputStream stream;
                            if (isAssets){
    ...
    public static HashMap<String,MtlInfo> readMtl(InputStream stream){
        ...
        try{
            ...
            while((temps=br.readLine())!=null){
                ...
                switch (tempsa[0].trim()){
                    case "newmtl":  //材质
                        LogUtils.i("readMtl  D : newmtl材质:"+temps);
                        mtlInfo=new MtlInfo();
                        mtlInfo.newmtl=tempsa[1];
                        map.put(tempsa[1],mtlInfo);
                        break;
                    case "illum":     //光照模型
                        LogUtils.i("readMtl  D : 光照模型:"+temps);
                        mtlInfo.illum=Integer.parseInt(tempsa[1]);
                        break;
                    case "Kd":
                        LogUtils.i("readMtl  D : Kd模型:"+temps);
                        read(tempsa,mtlInfo.Kd);
                        break;
                    case "Ka":
                        LogUtils.i("readMtl  D : Ka模型:"+temps);
                        read(tempsa,mtlInfo.Ka);
                        break;
                    case "Ke":
                        LogUtils.i("readMtl  D : Ke模型:"+temps);

                        read(tempsa,mtlInfo.Ke);
                        break;
                    case "Ks":
                        LogUtils.i("readMtl  D : Ks模型:"+temps);
                        read(tempsa,mtlInfo.Ks);
                        break;
                    case "Ns":
                        LogUtils.i("readMtl  D : Ns模型："+temps);
                        mtlInfo.Ns=Float.parseFloat(tempsa[1]);
                        break;
                    case "map_Kd":
                        LogUtils.i("readMtl  D : 纹理图："+temps);
